...
module.exports = {
    name: 'unblacklist',
    category: 'ticket',
    description: 'Hiermee kan je de rol "Blacklist" weghalen bij de persoon die je tagt.',
    usage: `unblacklist <@persoon> [reden]`,
    run: async (client, message, args) => {
        ...
        if (settings.Engels === true) {
            ...
            member.send(`🎫You are \`unblacklisted\` in **${message.guild.name}** \n**Reason**: ${reason}.`);
            await (member.roles.remove(Blacklisted));
            message.channel.send(`${member} is **unblacklisted**!`);
            if (!logChannel) {
                return
            } else {
                const embed = new MessageEmbed()
                    .setColor('#ff0000')
                    .setTitle('Person Unblacklisted')
                    .setThumbnail(member.user.avatarURL())
                    .addField('Person', member.user.username)
                    .addField('ID person', member.id)
                    .addField('Unblacklisted by', message.author)
            ...
            member.send(`🎫Je bent \`geunblacklist\` in **${message.guild.name}** \n**Reden**: ${reason}.`);
            await (member.roles.remove(Blacklisted));
            message.channel.send(`${member} is **geunblacklist**!`);
            if (!logChannel) {
                return
            } else {
                const embed = new MessageEmbed()
                    .setColor('#ff0000')
                    .setTitle('Persoon Geunblacklist')
                    .setThumbnail(member.user.avatarURL())
                    .addField('Persoon', member.user.username)
                    .addField('ID persoon', member.id)
                    .addField('Geunblacklist door', message.author)
