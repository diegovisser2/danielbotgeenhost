...
module.exports = {
    name: 'add',
    category: 'ticket',
    description: 'Hiermee kunt u iemand toevoegen in de ticket.',
    usage: `add <@persoon>`,
    run: async (client, message, args) => {
        ...
        if (settings.Engels === true) {
            ...
            var toegevoegdEMbed = new MessageEmbed()
                ...
                .setDescription(`✅ ${addUser} added by ${message.author}.`)
                .setColor("GREEN")
                .setTimestamp()
            ...
            const categoryID = guildDB.ticketMap;
            if (!categoryID)
                return message.channel.send("Het mapje waar tickets in moeten komen is niet door de owner ingesteld.");

            if (!message.member.roles.cache.find(r => r.name === 'Support Team'))
                return message.reply('Uw rol is niet "Support Team". Let op de hoofdletters mocht deze rol wel bestaan.');

            if (message.channel.parentID != categoryID)
                return message.reply('Deze commando kan alleen gebruikt worden in het ingestelde ticket mapje.')

            const addUser = message.guild.member(message.mentions.users.first() || message.guild.members.cache.get(args[0]));

            if (!addUser)
                return message.reply("Er is geen gebruiker opgegeven.");

            var toegevoegdEMbed = new MessageEmbed()
                // .setTitle("Gebruiker toegevoegd")
                .setDescription(`✅ ${addUser} toegevoegd door ${message.author}.`)
                .setColor("GREEN")
                .setTimestamp()
            // .addField("Toegevoegde gebruiker", `${addUser}`)
            // .addField("Toegevoegd door", message.author);
